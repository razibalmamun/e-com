						...
						<table class="min-w-full divide-y divide-gray-200">
							...
							<tr v-for="(order, key) in orders" :key="key">
								...
								<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
									<NuxtLink :to="localePath({name: 'admin-orders-id-edit', params: {id: order.id}})" class="text-indigo-600 hover:text-indigo-900">View</NuxtLink>
	...
	methods: {
		...
		deleteRecord(deleteId, index) {
			this.$confirm(
				...
				callback: confirm => {
					if (confirm) {
						this.$axios.delete("/v1/admin/orders/" + deleteId).then((response) => {
							if (response) {
